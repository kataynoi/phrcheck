...
- **Data scoping** is centralised in `App\Libraries\DataScope`, built from the session by `BaseController::scope()`. Three roles: `admin` (whole province), `district` (every unit sharing the user's `hospitals.distcode`), `user` (own `hoscode`). A district user's scope is derived from their own unit's district — there is no separate column on `users`, so `Admin\Users::setRole()` refuses to grant `district` to someone who has not picked a service unit yet.

  `DataScope::sqlCondition()` emits a subquery (`code IN (SELECT hoscode FROM hospitals WHERE distcode = ?)`) rather than an `IN` list so it stays correct when units move between districts; `allowedCodes()` returns the resolved list for the importer, which checks membership per row. Every query path funnels through one of these two — `EncounterMaskModel::scoped()`, `Dashboard::scopeSql()`, `UploadBatchModel::recentInScope()`, `EncounterImporter::import()`. Bulk updates re-apply the condition in the `WHERE` clause rather than trusting posted ids.

  Adding a role means touching `DataScope` and the `users.role` enum only. Enum changes need a hand-run script in `docker/mysql/upgrades/` for databases that already have data — `docker-entrypoint-initdb.d` never re-runs.
